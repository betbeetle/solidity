    ...
    // Enable optimization = 1 when deploying
    ...
    */
    ...
    contract Betting {
        ...
        mapping (uint256 => uint256) bets_total;
		...
		address beetle = 0x5f588B4Cc001fbB1A572eD5AaEA265BfEA251385;
		uint256 public beetleMin;
		...
		event BeetleSet(uint256 _amount);
        ...
        */
        ...
        /**
        * @dev Specify minimum beetle balance
        * @param _amount amount
        */
        function setMinBeetle(uint256 _amount) public onlyAdmin {
            beetleMin = _amount;
			emit BeetleSet(_amount);
        }
        ...
        */
        function sendBet(uint256 _betId, uint256 _option, uint256 _amount) public payable {
            ...
            if (bets[_betId].tokenAddress == bnbContract){
                if(bnbBalance[msg.sender] >= minBetAmount[bets[_betId].tokenAddress] && _amount <= bnbBalance[msg.sender]){
                    bnbBalance[msg.sender] = bnbBalance[msg.sender] - _amount;
                }
            }else{
                bep20Balances[msg.sender][bets[_betId].tokenAddress] = (bep20Balances[msg.sender][bets[_betId].tokenAddress]) - _amount;
            }
            ...
            bets_total[_betId] = bets_total[_betId].add(bet_amount);
        ...
        */
         function getPositivesBets(uint cursor, uint howMany) public view returns(Bet[] memory bet, uint newCursor) {
            uint256 length = bets.length;
            uint256 count;
            uint256 j;
            uint howmanylength = howMany;
            
            for (uint i = 0; i < length; i++) {
                if (activeBets[i] && (bets_total_option0[i] + bets_total_option1[i] > 0)){
                    count = count + 1;
                }
                if (count == howMany) {
                    break;
            ...
            if (count > 0){
                Bet[] memory _bets = new Bet[](count);
                for (uint i = 0; i < length; i++) {
                    if (activeBets[i] && (bets_total_option0[i] + bets_total_option1[i] > 0)) {
                        _bets[j] = bets[i];
                        j = j + 1;
                ...
                if (howmanylength > _bets.length - cursor) {
                    howmanylength = _bets.length - cursor;
                }
                Bet[] memory values = new Bet[] (howmanylength);
                for (uint i = 0; i < howmanylength; i++) {
                    values[howmanylength - i - 1] = _bets[cursor + i];
                }
                return (values, cursor + howmanylength); 
            }
        }
    
        /**
        * @dev get a specific bet.
        * @param _betId.
        **/
        function getBet(uint256 _betId) public view returns (Bet memory bet){
            if(activeBets[_betId]) {
                return bets[_betId];
            }
        }
        
        /**
        * @dev Check for expired bets
        */
        function updateBetsStates() public {
            uint256 length = bets.length;
            for (uint256 bind = 0; bind < length; bind++){
                if (bets[bind].endDate <= block.timestamp){
                    activeBets[bind] = false;
                }
            }
        }
        ...
        */
		...
		/**
        * @dev get number of beetle
		* @param _address holder
        */
        function getBeetleBalance(address _address) public view returns(uint) {
			BEP20 _beetle = BEP20(beetle);
			return _beetle.balanceOf(_address);
        }
        ...
        */
        function withdraw(address _token) public {
			...
			require(_beetle.balanceOf(msg.sender) >= beetleMin, 'Not enough BEETLE');
